...
export function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const navItems = [
    { icon: Cloud, label: 'All Files', href: '/dashboard' },
      ...
      <aside className="island-shell m-4 mr-0 flex w-72 flex-col rounded-[2rem] border-r-0">
        <div className="p-8">
          <Link to="/dashboard" className="flex items-center gap-3 no-underline">
            ...
            <span className="display-title text-xl font-bold text-[var(--sea-ink)]">CloudVault</span>
          </Link>
        </div>

        <nav className="flex-1 space-y-2 px-4">
          <p className="px-4 text-[10px] font-bold tracking-[0.2em] text-[var(--sea-ink-soft)] uppercase">Main Menu</p>
          {navItems.map((item) => (
            <Link
              key={item.label}
              to={item.href}
              ...
              activeProps={{
                className: 'bg-[var(--lagoon)]/15 text-[var(--sea-ink)] shadow-sm',
              }}
            >
              <item.icon size={20} />
              {item.label}
          ...
          <p className="px-4 text-[10px] font-bold tracking-[0.2em] text-[var(--sea-ink-soft)] uppercase">Preferences</p>
            ...
            <DropdownMenuTrigger asChild>
              <button className="flex w-full items-center gap-3 rounded-2xl bg-white/40 p-3 text-left transition-all hover:bg-white/60 hover:shadow-md focus:outline-none">
                ...
                <div className="flex-1 overflow-hidden">
                  <p className="truncate text-sm font-bold text-[var(--sea-ink)]">{user?.name || 'User'}</p>
                  <p className="truncate text-[11px] text-[var(--sea-ink-soft)]">{user?.email || 'user@example.com'}</p>
                </div>
                <ChevronRight size={16} className="text-[var(--sea-ink-soft)]" />
              </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-56 rounded-2xl border-[var(--line)] p-2 shadow-xl" side="right" sideOffset={12}>
              <DropdownMenuLabel className="px-3 py-2 text-xs font-bold tracking-wider text-[var(--sea-ink-soft)] uppercase">My Account</DropdownMenuLabel>
